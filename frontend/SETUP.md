...
### ⚠️ Важно: Работа с моками

**На данный момент все API запросы используют моки (тестовые данные).**

Это позволяет протестировать интерфейс без запущенного backend.

- Для входа используйте любой email и пароль (валидация работает)
- Все данные на дашборде и в истории - тестовые
- Подробная инструкция по переключению на реальные API: [`src/api/README_MOCKS.md`](src/api/README_MOCKS.md)
- Все места для изменения помечены комментарием `// TODO: когда будет готов бэкенд`
